...
function CardProfileLike(props) {
  ...
  const handleUpdateItem = useCallback((data) => {
    if (data) {
      setUpdateItem({ ...data })
    }
  }, [])
  ...
  useEffect(() => {
    if (isInitialized && (walletAddress || props.address)) {
      (async () => {
        fetchIndexRef.current ++
        const fetchIndex = fetchIndexRef.current
        try {
          const response = await Moralis.Cloud.run('getProfileLikeNFTs', {
            walletAddress: props.address? props.address: walletAddress
          });
          if (response.result && response.data && fetchIndex === fetchIndexRef.current) {
            setLikeNfts(response.data)
          }
        } catch (err) {
  ...
  }, [Moralis.Cloud, Moralis.Object, Moralis.Query, isInitialized, props.address, walletAddress])
  ...
  }, [allCollections, chainId, getContractInstance, getCollectionAddresses, getCollectionWithAddress, likeNfts, ownerAddress, walletAddress])
...
export default React.memo(CardProfileLike);

...
import { getAuctionListItems, getListItemDetail } from '../../apis/marketplace';
...
function CardMarketplace(props) {
  ...
  const { data, isLoading, error } = useQuery('auctionListItems', getAuctionListItems)
    ...
    if (updateItem && cardItems) {
      ...
      if (itemToUpdate) {
        if (updateItem.isOnSale === false) {
          const newItems = cardItems.filter((item) => (item.id!==updateItem.id || item.collectionAddress!==updateItem.collectionAddress))

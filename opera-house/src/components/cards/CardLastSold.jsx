...
import { getAllListItems, getLastSold, getListItemDetail } from '../../apis/marketplace';
  ...
  useEffect(() => {
    if (isInitialized) {
      (async () => {
        ...
        try {
          const result = await getLastSold()
  ...
  useEffect(() => {
    if (likeNfts && allCollections?.length) {
      ...
      (async () => {
        ...
        const queryData = likeNfts.map(({ nftContract, tokenId }) => ({ id: tokenId, collectionAddress: nftContract }))
        const nfts = likeNfts.map(({ nftContract, tokenId, price, paymentToken  }) => ({ id: tokenId, collectionAddress: nftContract, soldPrice: price, soldPaymentToken: paymentToken }))
        ...
        for (let index = 0; index < itemsWithoutMetadata.length; index++) {
          const element = itemsWithoutMetadata[index];
          fetchMetadata(element)
        }
      })()
    }
...
export default React.memo(CardLastSold);

...
import { getEndedAuctionListItems, getListItemDetail } from '../../../apis/marketplace';
...
function CardMarketplace(props) {
  ...
  const { data, isLoading, error } = useQuery('endedAuctionListItems', getEndedAuctionListItems)
  ...
  const handleAcceptBid = useCallback(async (cardItem, highestBid) => {
    try {
      ...
      const { bidder, price, amount } = highestBid
      ...
      buyFunc = marketplaceInstance.methods.acceptBid(collectionAddress, contractType, id, amount, bidder, price)

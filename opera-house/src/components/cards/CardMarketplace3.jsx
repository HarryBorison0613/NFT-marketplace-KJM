...
function CardMarketplace(props) {
  ...
  }, [walletAddress, to, getContractInstance])
  ...
  const handleSetSellPendingOther = React.useCallback(async (cardItem) => {
    ...
    try{
      await NFTContractInstance.methods.approve(defaultCollectionAddress, cardItem.id).send({ from: walletAddress });
      isApproved = true;
    } catch(err) {
      console.log(err);
    }
  ...
  }, [sellPrice, getContractInstance, walletAddress, history])
  ...
  }, [handleSetSellPendingOther, sellPrice, getContractInstance, walletAddress, history])
  ...
  }, [getContractInstance, history, walletAddress, handleRemoveSellPendingOther])
        ...
        if(collectionAddress && collectionAddress !== "profile") {  //collection page
          ...
          if(collectionAddress === defaultCollectionAddress) { //about default collection
            ...
            const fetchMetadata = async (data) => {
              try {
                if (data) {
                  const { token_uri, token_id: id, metadata: _metadata, name: _name } = data
                  ...
                  let cardItem = {};
                  let metadata = _metadata ? JSON.parse(_metadata): null
                  let isBurnedNFT = false
                  cardItem.isOwner = false;
                  ...
                  if (!metadata) {
                    console.log('no metadata', _metadata)
          ...
          if(isProfile) {
            ...
            if (total > 0) {
              ...
              await Promise.all(nfts.map((data) => getItemMetaData(data)))
            }
            // const totalLength = networkCollections["0xfa"].length
            // let _index = 0;
            // while(true) {
            //   const startId = _index
            //   if (_index < totalLength - 5) _index += 5
            //   else if (_index < totalLength) _index = totalLength
            //   else break
            //   const promises = []
            //   for(let i = startId; i < _index; i++) {
  ...
  // useEffect(() => {
    // if (!traitFilterOptions) setCardItemsToShow(_cardItemsToShow)
    // else {
    //   const newItems = _cardItemsToShow.filter(({ attributes }) => {
    //     if (!attributes) return false
    //     let result = true
    //     traitFilterOptions.forEach(({ type, value }) => {
    //       const attr = attributes.find((item) => {
    ...
    <div>
        <div className="row mb-50_reset mx-auto text-center">
          ...
          {_cardItemsToShow.map((cardItem, i) => (
            <div className="col-xl-3 col-lg-3 col-md-6 col-sm-12" key={cardItem.id}>

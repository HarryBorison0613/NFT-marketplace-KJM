...
contract OperaHouseV2 is ReentrancyGuard, Ownable {
    ...
    uint256 _serviceFee = 10;  // 0 % with 1000 factor
    ...
    function _listTokenERC1155(
        ...
        if (!listType) require(amount == 1, "Only 1 allowed for auction");
    ...
    function listTokenERC1155(
        ...
        if (listingFee > 0) {
            require(msg.value >= listingFee, "Listing fee error");
    ...
    function bulkListToken(
        ...
        for (uint256 i = 0; i < listItems.length; i ++) {
            if (listItems[i].contractType == 1) {
                ...
                _listTokenERC1155(listItems[i].nftContract, listItems[i].tokenId, listItems[i].amount, listItems[i].price, listItems[i].paymentToken, listItems[i].listType, listItems[i].expireTimestamp);
            } else {
    ...
    function buyTokenWithETH(
        ...
        (uint256 totalPrice, uint256 royaltyPrice, address recipient, uint256 serviceFee) = _beforeBuyToken(nftContract, tokenId, listItem.price, amount);
    ...
    function enterBid(
        ...
        public onlyMarketplaceOpen
    {
        Bid memory bid = Bid(tokenId, price, msg.sender);

        require(_marketplaceSales[nftContract].tokenIdsListing.contains(tokenId), "Not for bid");

        ListItem storage listItem = _marketplaceSales[nftContract].listings[tokenId];
        
        if ((listItem.contractType == 1 && !_isBidValid(nftContract, bid))) {
            revert("Bid is not valid");
        }
        require(listItem.listType == false, "It is not on auction");
        require(listItem.expireTimestamp > block.timestamp, "Auction time expired");
        ...
        require(price > listItem.price, "Wrong price");
    ...
    function acceptBid(
        ...
        if (contractType == 1) {
            ...
            require(
                _isTokenOwnerERC1155(nftContract, tokenId, 1, msg.sender),
                "Only token owner can accept bid of token"
            );
            require(
            ...
            require(_isBidValidERC1155(bid), "Not valid bidder");
        }
        ...
        require(price >= listItem.price, "Not highest bid");

        (uint256 totalPrice, uint256 royaltyPrice, address recipient, uint256 serviceFee) = _beforeBuyToken(nftContract, tokenId, price, 1);
        ...
        if (listItem.contractType == 1) {
            ...
            IERC1155(nftContract).safeTransferFrom(listItem.seller, bidder, tokenId, 1, "");
        }
        ...
        _delistToken(nftContract, tokenId, 1);
    }

    function transfer(address nftContract, uint256 tokenId, uint contractType, address toAccount, uint256 amount)
        external
    {
        if (contractType == 1) {
            IERC721(nftContract).safeTransferFrom(msg.sender, toAccount, tokenId);
        } else {
            IERC1155(nftContract).safeTransferFrom(msg.sender, toAccount, tokenId, amount, "");
        }
        emit TokenDelisted(
            nftContract,
            tokenId,
        ...
        external
    {
        for (uint256 i = 0; i < items.length; i ++) {
            ...
            if (item.contractType == 1) {
                ...
                IERC1155(item.nftContract).safeTransferFrom(msg.sender, item.toAccount, item.tokenId, amount, "");
            }
            emit TokenDelisted(
                item.nftContract,
                item.tokenId,
            ...
            _delistToken(item.nftContract, item.tokenId, amount);
        }
    }
    ...
    {
        ListItem memory listItem = _marketplaceSales[nftContract].listings[tokenId];
        highestBid = Bid(tokenId, 0, address(0));
        uint256 bidderCount = _marketplaceSales[nftContract]
            .bids[tokenId]
            .bidders
            .length();
    ...
    function setRoyalty(
        ...
        address recipient;
        if (_collectionRoyalty[collectionAddress].setBy != address(0)) {
            recipient = _collectionRoyalty[collectionAddress].recipient;
        }
        if (msg.sender != owner()) {
            require(

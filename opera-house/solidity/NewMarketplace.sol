...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function paymentTokens() external view onlyOwner returns(address[] memory) {
    ...
    function buyTokenWithETH(
        ...
        (uint256 totalPrice, uint256 royaltyPrice, address recipient, uint256 serviceFee) = _beforeBuyToken(nftContract, tokenId, amount, listItem);
    ...
    function accpetBid(
        ...
        require(IERC20(paymentToken).allowance(bid.bidder, address(this)) >= bid.price &&
            IERC20(paymentToken).balanceOf(bid.bidder) >= bid.price,
        ...
        if (checkRoyalties(nftContract)) {
            ...
            CollectionRoyalty memory collectionRoyalty = royalty(nftContract);
            recipient = collectionRoyalty.recipient;
            if (recipient != address(0)) royaltyPrice = totalPrice.mul(collectionRoyalty.feeFraction).div(1000);
        }

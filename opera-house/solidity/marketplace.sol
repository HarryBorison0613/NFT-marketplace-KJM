...
contract Marketplace is ReentrancyGuard, Ownable, OperahouseMarketplaceRoyalty {
    ...
    address swapRouter = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;
    ...
    function buyToken(
        ...
        if (listItem.paymentToken == address(0)) {
            ...
            IERC20(listItem.paymentToken).safeTransfer(listItem.seller, totalPrice - royaltyPrice - serviceFee);
        }
    ...
    function buyTokenWithOtherToken(
        address nftContract,
        uint256 tokenId,
        uint256 amount,
        address paymentToken,
        ...
        IERC20(listItem.paymentToken).safeTransfer(address(this), maxTokenAmount);
        ...
        if (listItem.paymentToken == address(0)) {
            ...
            IUniswapV2Router(swapRouter).swapTokensForExactETH(
                totalPrice,
                maxTokenAmount,
                path,
                address(this),
            ...
            uint256 afterBal = IERC20(paymentToken).balanceOf(address(this));
            if (afterBal > 0) {
                IERC20(paymentToken).safeTransferFrom(address(this), msg.sender, afterBal);
            }
            if (recipient != address(0)) {
                payable(recipient).transfer(royaltyPrice);
            }
            payable(adminAddress).transfer(serviceFee);
            payable(listItem.seller).transfer(totalPrice - royaltyPrice - serviceFee);
        } else {
            address[] memory path;
            path = new address[](3);
            path[0] = paymentToken;
            ...
            IUniswapV2Router(swapRouter).swapTokensForExactTokens(
                totalPrice,
                maxTokenAmount,
                path,
                address(this),
    ...
    function buyTokenWithETH(
        ...
        IUniswapV2Router(swapRouter).swapETHForExactTokens(
            totalPrice,
            path,
            address(this),
            block.timestamp
        ...
        if (afterBal > 0) {
            payable(msg.sender).transfer(afterBal);
        }
